...
//			http.authorizeRequests().antMatchers("/mf", "/mf/",
//					"/mf/loginFailed.html").permitAll().antMatchers("/mf/**").hasRole(
//					SiteWhereRoles.AUTH_ADMIN_CONSOLE).anyRequest().authenticated();
//			http.formLogin().loginPage("/mf/").loginProcessingUrl("/mf/login.html").defaultSuccessUrl(
//					"/mf/test.html").failureUrl("/mf/loginFailed.html");
...
//
	...
	public static class MFSecurityConfiguration extends WebSecurityConfigurerAdapter {
		...
		protected void configure(HttpSecurity http) throws Exception {
			http.csrf().disable();
			http.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().antMatcher(
					"/mf/**").authorizeRequests().antMatchers(HttpMethod.OPTIONS,
					"/mf/**").permitAll().antMatchers(HttpMethod.GET,
					"/mf/**/symbol").permitAll().antMatchers("/api/**").hasRole(
					SiteWhereRoles.AUTH_REST).and().httpBasic();
		}
	}
